...
export const jobService = {
  ...
  async getUserJobLogs(
    ...
    try {
      ...
      if (paginatedLogs.length > 0) {
        const postIds = paginatedLogs.filter(log => log.postId).map(log => log.postId as string);
  ...
  async retryJob(jobLogId: string, userId: string): Promise<JobLog | null> {
    ...
    try {
      ...
      // Update the job log
      jobLog.jobId = String(job.id); // Ensure job.id is a string
